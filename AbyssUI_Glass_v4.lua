...
local function saveConfiguration()
    ...
    if configSettings.SavePosition and Library.Window and not Library.Window._destroyed then
        ...
        if main then
            ...
            local saveHeight = window._minimized and (window._restoredHeight or main.Size.Y.Offset) or main.Size.Y.Offset
            data.__AbyssUI.Size = { X = main.Size.X.Offset, Y = saveHeight }
            data.__AbyssUI.RestoredHeight = window._restoredHeight
            data.__AbyssUI.Minimized = window._minimized == true
            data.__AbyssUI.Theme = currentThemeName()
        end
    end
...
function Library:LoadConfiguration()
    ...
    if type(meta) == "table" and meta.Theme then
        applyThemePreset(meta.Theme)
        if Library.SetTheme and Library.Window and not Library.Window._destroyed then
            Library:SetTheme(meta.Theme)
        end
    end
    ...
    if type(meta) == "table" and meta.Minimized and Library.Window and not Library.Window._destroyed then
...
function Library:CreateWindow(settings)
    ...
    if settings.ConfigurationSaving then
        ...
        if settings.ConfigurationSaving.AutoLoad ~= nil then
            configSettings.AutoLoad = settings.ConfigurationSaving.AutoLoad == true
        end
    end
    ...
    local function refreshCommandList(query)
        ...
        for _, entry in ipairs(commandEntries()) do
            if lowered == "" or entry.Name:lower():find(lowered, 1, true) then
                ...
                commandMaid:Connect(button.MouseLeave, function()
                    tween(button, 0.14, { BackgroundTransparency = 0.20 })
                end)
                commandMaid:Connect(button.MouseButton1Click, function()
                    entry.Action()
    ...
    WindowMaid:Connect(commandInput.InputBegan, function(input)
        if input.KeyCode == Enum.KeyCode.Escape then setCommandPalette(false) end
    end)
    ...
    WindowMaid:Connect(dragZone.MouseButton1Down, function()
        dragging = true
    ...
    function Library:SetAccent(color, color2)
        ...
        for _, tab in ipairs(Window.Tabs) do
            for _, element in ipairs(tab.elements) do
                if element.frame and element.frame.Parent then
                    if element.frame:FindFirstChild("ElementGlassStroke", true) then
                        -- marker reserved for future theme-aware children
                    end
                end
            end
        end
    ...
    function Library:SetTheme(name)
        ...
        for _, tab in ipairs(Window.Tabs) do
            for _, element in ipairs(tab.elements) do
                ...
                if f and f.Parent then
                    if f:GetAttribute("AbyssElement") then
        ...
        for _, obj in ipairs(screenGui:GetDescendants()) do
            if obj:IsA("Frame") and obj:GetAttribute("AbyssElement") then
                obj.BackgroundColor3 = Theme.ElementBackground
            elseif obj:IsA("TextLabel") and (obj.Name == "TabName" or obj.Name == "TabIcon") then
                obj.TextColor3 = Theme.TabTextColor
    ...
    if settings.ConfigurationSaving and configSettings.Enabled and configSettings.AutoLoad then
        task.defer(function()
            if not Window._destroyed then
