...
local function ensureRootGui()
    ...
    if oldGui then
        local shutdown = oldGui:FindFirstChild("AbyssShutdown")
        if shutdown and shutdown:IsA("BindableEvent") then
            pcall(function() shutdown:Fire() end)
            task.wait()
        end
    ...
    new("BindableEvent", { Name = "AbyssShutdown" }, screenGui)
    ...
    if type(meta) == "table" then
        ...
        if type(meta.UIState) == "table" then
            self:SetAccessibility({
                ReducedMotion = meta.UIState.ReducedMotion == true,
                HighContrast = meta.UIState.HighContrast == true,
                Density = meta.UIState.Density == "Compact" and "Compact" or "Comfortable",
                Scale = math.clamp(tonumber(meta.UIState.Scale) or 1, 0.85, 1.25),
            })
        end
        if Library.Window and not Library.Window._destroyed then
            ...
            if meta.Minimized then
                ...
                Library.Window:SetMinimized(false, false)
            end
        end
    end
...
function Library:CreateWindow(settings)
    ...
    local shutdownEvent = screenGui:FindFirstChild("AbyssShutdown")
    if shutdownEvent and shutdownEvent:IsA("BindableEvent") then
        WindowMaid:Connect(shutdownEvent.Event, function()
            if Library.Window == Window and Window and not Window._destroyed then
                Window:Destroy()
            end
        end)
    end
    ...
    local commandBox = new("Frame", {
        ...
        Size = UDim2.fromOffset(math.min(520, math.max(300, screenGui.AbsoluteSize.X - 16)), 330),
        BackgroundColor3 = Theme.Background,
    ...
    local function queueConfigSave()
        ...
        task.delay(0.20, function()
            if generation ~= lifecycleGeneration then return end
            positionSaveQueued = false
            if not Window._destroyed then saveConfiguration() end
        end)
    end
    ...
    function Window:SetPosition(x, y, save)
        if Window._destroyed or not main.Parent then return false end
    ...
    function Window:SetSize(width, height, save)
        if Window._destroyed or not main.Parent then return false end
        local viewport = screenGui.AbsoluteSize
        local responsiveMinW = math.min(Theme.MinWidth, math.max(300, viewport.X - 20))
        local responsiveMinH = math.min(Theme.MinHeight, math.max(240, viewport.Y - 20))
        local w = math.clamp(tonumber(width) or main.Size.X.Offset, responsiveMinW, math.min(Theme.MaxWidth, math.max(responsiveMinW, viewport.X - 20)))
        local h = math.clamp(tonumber(height) or main.Size.Y.Offset, responsiveMinH, math.min(Theme.MaxHeight, math.max(responsiveMinH, viewport.Y - 20)))
        if Window._minimized then
            Window._restoredHeight = h
            main.Size = UDim2.fromOffset(w, 66)
        else
            main.Size = UDim2.fromOffset(w, h)
            Window._restoredHeight = h
        end
        local nx, ny = clampWindowOffsets(main.Position.X.Offset, main.Position.Y.Offset)
        main.Position = UDim2.new(0.5, nx, 0.5, ny)
        setShadowForMain()
        if save ~= false then queueConfigSave() end
        return true
    ...
    function Window:SetGlassTransparency(value)
        if Window._destroyed or not screenGui or not screenGui.Parent then return false end
    ...
    local function attachFloatingPopup(popup, anchor, onClose)
        ...
        Window._popupClosers[#Window._popupClosers + 1] = closePopup
        WindowMaid:Give(function()
            for i = #Window._popupClosers, 1, -1 do
                if Window._popupClosers[i] == closePopup then
                    table.remove(Window._popupClosers, i)
                    break
                end
            end
        end)
        local function reposition()
            ...
            local targetWidth = math.max(160, as.X)
            local currentHeight = popup.AbsoluteSize.Y > 0 and popup.AbsoluteSize.Y or popup.Size.Y.Offset
            if currentHeight <= 0 then currentHeight = 40 end
            popup.Size = UDim2.fromOffset(targetWidth, currentHeight)
            local w = targetWidth
    ...
    function Window:CreateTab(tabNameOrOptions)
        ...
        function Tab:CreateParagraph(opts)
            ...
            return {
                Set = function(_, value)
                    value = value or {}
                    title.Text = tostring((value.Title ~= nil and value.Title or title.Text))
        ...
        function Tab:CreateDropdown(opts)
            ...
            setOpen = function(state)
                ...
                tween(list, 0.20, { Size = UDim2.fromOffset(math.max(160, frame.AbsoluteSize.X), open and contentHeight or 0) })
        ...
        function Tab:CreateColorPicker(opts)
            ...
            Window._popupClosers[#Window._popupClosers + 1] = closeColorPopup
            WindowMaid:Give(function()
                for i = #Window._popupClosers, 1, -1 do
                    if Window._popupClosers[i] == closeColorPopup then table.remove(Window._popupClosers, i) break end
                end
            end)
            WindowMaid:Connect(swatch.MouseButton1Click, function()
                if popup.Visible then closeColorPopup() else Window:ClosePopups(); popup.Visible = true; positionPopup() end
            end)
            WindowMaid:Connect(UserInputService.InputBegan, function(input)
                if not popup.Visible or (input.UserInputType ~= Enum.UserInputType.MouseButton1 and input.UserInputType ~= Enum.UserInputType.Touch) then return end
                local p = input.Position
    ...
    function Window:ClosePopups()
        if Window._destroyed then return end
        for i = #Window._popupClosers, 1, -1 do
            local closer = Window._popupClosers[i]
            pcall(closer)
        end
        for _, obj in ipairs(screenGui:GetDescendants()) do
    ...
    if settings.BuiltInTabs ~= false then
        ...
        settingsTab:CreateSlider({ Name = "UI scale", Range = { 85, 125 }, Min = 85, Max = 125, Increment = 1, CurrentValue = math.floor(UIState.Scale * 100), Suffix = "%", Flag = "ui.scale", Callback = function(v) Library:SetAccessibility({ Scale = v / 100 }) end })
    ...
    local function applyResponsiveLayout()
        if Window._destroyed or not screenGui.Parent then return end
        local vp = screenGui.AbsoluteSize
        local mobile = vp.X < 560
        local width = math.min(Theme.MaxWidth, math.max(300, vp.X - 20))
        local height = math.min(Theme.MaxHeight, math.max(240, vp.Y - 20))
        if mobile then
            sidebar.Size = UDim2.fromOffset(56, math.max(180, screenGui.AbsoluteSize.Y - 82))
            sidebar.Position = UDim2.fromOffset(8, 72)
            pages.Size = UDim2.new(1, -72, 1, -82)
            pages.Position = UDim2.fromOffset(64, 72)
            searchBox.Size = UDim2.fromOffset(math.max(92, math.min(145, vp.X - 180)), 30)
        else
            sidebar.Size = UDim2.new(0, UIState.Density == "Compact" and 146 or 154, 1, -82)
            sidebar.Position = UDim2.fromOffset(10, 72)
            pages.Size = UDim2.new(1, -(UIState.Density == "Compact" and 172 or 180), 1, -82)
            pages.Position = UDim2.fromOffset(UIState.Density == "Compact" and 162 or 170, 72)
            searchBox.Size = UDim2.fromOffset(145, 30)
        end
        for _, rec in ipairs(Window.Tabs) do
            if rec.nameLabel then rec.nameLabel.Visible = not mobile end
            if rec.iconLabel then rec.iconLabel.Position = mobile and UDim2.fromOffset(14, 0) or UDim2.fromOffset(10, 0) end
        end
        Window:SetSize(math.min(main.Size.X.Offset, width), math.min(Window._restoredHeight, height), false)
        commandBox.Size = UDim2.fromOffset(math.min(520, math.max(300, vp.X - 16)), math.min(330, math.max(220, vp.Y - 24)))
        commandBox.Position = UDim2.new(0.5, 0, 0, math.max(8, math.min(100, vp.Y - commandBox.AbsoluteSize.Y - 8)))
    end
