...
local Settings = {
    ...
    Misc   = { AntiAFK = true, Fullbright = false, Watermark = true, PlayerList = false, InputLock = false, RemoteConfigURL = "" },
...
local function tween(o, t, pr)
    if not o or not o.Parent then return end
    TweenService:Create(o, TweenInfo.new(t or 0.16, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), pr):Play()
end
...
local function isEnemy(p)
    ...
    local m, t = LocalPlayer.Team, p.Team
    if not m or not t then return true end
    if LocalPlayer.Neutral or p.Neutral then return true end
    return t ~= m
end
local function getBestPart(c)
...
local function Acquire(cls)
    ...
    for i = 1, #pool do
        local o = pool[i]
        if Free[o] then Free[o] = false return o end
    end
...
----------------------------------------------------------------
local C = {
    bg = Color3.fromRGB(8, 8, 10), topbar = Color3.fromRGB(13, 13, 16),
    sidebar = Color3.fromRGB(11, 11, 14), element = Color3.fromRGB(19, 19, 23),
    hover = Color3.fromRGB(27, 27, 32), strokeC = Color3.fromRGB(38, 38, 44),
    text = Color3.fromRGB(238, 238, 242), muted = Color3.fromRGB(118, 118, 128),
    accent = Color3.fromRGB(255, 62, 62), off = Color3.fromRGB(58, 58, 66),
}
...
do
    ...
    if type(gethui) == "function" then
        ok, t = pcall(gethui)
        if ok and t then guiParent = t end
    end
    if not guiParent then
        ok, t = pcall(function() return LocalPlayer:WaitForChild("PlayerGui", 3) end)
        if ok and t then guiParent = t else guiParent = game:GetService("CoreGui") end
    end
end
for _, ch in ipairs(guiParent:GetChildren()) do
    if ch:GetAttribute("AbyssCheat") then pcall(function() ch:Destroy() end) end
end

local screenGui = new("ScreenGui", {
    Name = "ui" .. math.random(10000, 99999),
    ResetOnSpawn = false, IgnoreGuiInset = true,
    ZIndexBehavior = Enum.ZIndexBehavior.Sibling, DisplayOrder = 999,
}, guiParent)
screenGui:SetAttribute("AbyssCheat", true)
...
new("TextLabel", { Size = UDim2.fromOffset(220, 20), Position = UDim2.fromOffset(24, 13), BackgroundTransparency = 1, Text = "unknown", Font = Enum.Font.Michroma, TextSize = 15, TextColor3 = Color3.fromRGB(248, 248, 250), TextXAlignment = Enum.TextXAlignment.Left, Parent = topbar })
...
local sidebar = new("Frame", { Size = UDim2.new(0, 128, 1, -54), Position = UDim2.fromOffset(8, 52), BackgroundColor3 = C.sidebar, BorderSizePixel = 0, Parent = main })
corner(sidebar, 8)
...
local function createTab(name)
    ...
    corner(btn, 6)
    local bar = new("Frame", { Size = UDim2.fromOffset(2, 16), Position = UDim2.fromOffset(0, 8), BackgroundColor3 = C.accent, BorderSizePixel = 0, Visible = false, Parent = btn })
    ...
    local function elemFrame(h)
        local f = new("Frame", { Size = UDim2.new(1, 0, 0, h), BackgroundColor3 = C.element, BorderSizePixel = 0, LayoutOrder = nextOrder(), Parent = page })
        corner(f, 7)
        stroke(f, C.strokeC)
        return f
    end
    local function elemTitle(p, t)
        return new("TextLabel", { Size = UDim2.new(0.62, -12, 1, 0), Position = UDim2.fromOffset(10, 0), BackgroundTransparency = 1, Text = t, TextXAlignment = Enum.TextXAlignment.Left, TextColor3 = C.text, Font = Enum.Font.Gotham, TextSize = 13, Parent = p })
    end
    ...
    local function addClick(f, cb)
        local o = new("TextButton", { Size = UDim2.fromScale(1, 1), BackgroundTransparency = 1, Text = "", AutoButtonColor = false, ZIndex = 5, Parent = f })
        connect(o.MouseButton1Click, function() pcall(cb) end)
        return o
    end

    function Tab:CreateSection(t)
        new("TextLabel", { Size = UDim2.new(1, 0, 0, 18), BackgroundTransparency = 1, Text = "  " .. tostring(t):upper(), TextXAlignment = Enum.TextXAlignment.Left, TextColor3 = C.muted, Font = Enum.Font.GothamSemibold, TextSize = 11, LayoutOrder = nextOrder(), Parent = page })
    end
    function Tab:CreateLabel(t)
        local f = elemFrame(24) hoverize(f)
        local l = elemTitle(f, t) l.Size = UDim2.new(1, -14, 1, 0) l.TextColor3 = C.muted l.TextSize = 12
        return { Set = function(_, v) l.Text = tostring(v) end }
    end
    function Tab:CreateButton(o)
        o = o or {}
        local f = elemFrame(30) hoverize(f)
        local l = elemTitle(f, o.Name or "Button") l.Size = UDim2.new(1, -14, 1, 0)
        addClick(f, function() if type(o.Callback) == "function" then o.Callback() end end)
        return { Set = function(_, v) l.Text = tostring(v) end }
    end
    function Tab:CreateToggle(o)
        ...
        local f = elemFrame(30) hoverize(f)
        elemTitle(f, o.Name or "Toggle")
        local sw = new("Frame", { Size = UDim2.fromOffset(38, 18), Position = UDim2.new(1, -48, 0.5, -9), BackgroundColor3 = C.bg, BorderSizePixel = 0, Parent = f })
        corner(sw, 9) stroke(sw, C.strokeC)
        local ind = new("Frame", { Size = UDim2.fromOffset(12, 12), AnchorPoint = Vector2.new(0, 0.5), Position = state and UDim2.new(1, -15, 0.5, 0) or UDim2.new(0, 3, 0.5, 0), BackgroundColor3 = state and C.accent or C.off, BorderSizePixel = 0, Parent = sw })
        corner(ind, 6)
    ...
    function Tab:CreateSlider(o)
        o = o or {}
        local min = tonumber(o.Range and o.Range[1]) or 0
        local max = tonumber(o.Range and o.Range[2]) or 100
        ...
        local track = new("Frame", { Size = UDim2.new(1, -20, 0, 6), Position = UDim2.fromOffset(10, 30), BackgroundColor3 = C.off, BorderSizePixel = 0, Parent = f })
        corner(track, 3)
        ...
        local function apply(v, fire)
            ...
            v = math.clamp(min + math.floor((v - min) / inc + 0.5) * inc, min, max)
            value = v obj.CurrentValue = v
    ...
    function Tab:CreateDropdown(o)
        ...
        elemTitle(f, o.Name or "Dropdown")
        ...
        local list = new("Frame", { BackgroundColor3 = C.topbar, BorderSizePixel = 0, ClipsDescendants = true, Visible = false, ZIndex = 200, Parent = screenGui })
        corner(list, 7) stroke(list, C.strokeC)
        ...
        local function paint()
            for _, b in ipairs(buttons) do
                ...
                b.TextColor3 = isCur and C.accent or C.text
            end
        end
        local function place()
            if not list.Visible or not f.Parent then return end
            local ap, as = f.AbsolutePosition, f.AbsoluteSize
            list.Size = UDim2.fromOffset(math.max(as.X, 160), math.clamp(#options * 27 + 10, 26, 190))
        ...
        for _, opt in ipairs(options) do
            ...
            connect(b.MouseButton1Click, function()
                ...
                if type(o.Callback) == "function" then pcall(o.Callback, opt) end
                paint()
                open = false arrow.Text = "v" list.Visible = false
                queueSave()
            end)
        end
        connect(UserInputService.InputBegan, function(i)
            if open and i.UserInputType == Enum.UserInputType.MouseButton1 then
                ...
                local ap, as = f.AbsolutePosition, f.AbsoluteSize
                local inL = p.X >= lp.X and p.X <= lp.X + ls.X and p.Y >= lp.Y and p.Y <= lp.Y + ls.Y
                local inA = p.X >= ap.X and p.X <= ap.X + as.X and p.Y >= ap.Y and p.Y <= ap.Y + as.Y
                if not inL and not inA then open = false arrow.Text = "v" list.Visible = false end
            end
        end)
        addClick(f, function()
            open = not open
    ...
    function Tab:CreateKeybind(o)
        o = o or {}
        local f = elemFrame(30) hoverize(f)
        elemTitle(f, o.Name or "Keybind")
        local b = new("TextButton", { Size = UDim2.fromOffset(76, 20), Position = UDim2.new(1, -86, 0.5, -10), BackgroundColor3 = C.element, BorderSizePixel = 0, Text = tostring(o.CurrentValue or "None"), TextColor3 = C.text, Font = Enum.Font.Gotham, TextSize = 11, AutoButtonColor = false, Parent = f })
        corner(b, 5) stroke(b, C.strokeC)
        ...
        connect(UserInputService.InputBegan, function(input)
            if capturing then
                if input.KeyCode == Enum.KeyCode.Escape then
                    capturing = false
                    b.Text = tostring(o.CurrentValue or "None")
                    return
                end
                if input.KeyCode ~= Enum.KeyCode.Unknown then
                    ...
                    o.CurrentValue = n
                    b.Text = n
                    if type(o.Callback) == "function" then pcall(o.Callback, n) end
                    queueSave()
                end
            end
        end)
...
if drawingAvailable then
    ...
    if ok and c then
        fovCircle = c
        c.Thickness = 2 c.NumSides = 48 c.Filled = false c.Color = C.accent c.Visible = false
        table.insert(extraDrawings, c)
    end
end
...
local function updateAim(dt)
    ...
    local S = Settings.Aimbot
    if fovCircle then
        if (S.Enabled or S.Silent) and S.ShowFOVCircle and Camera then
            local vp = Camera.ViewportSize
...
bindRender("ui_aim_" .. math.random(10000, 99999), Enum.RenderPriority.Camera.Value + 1, updateAim)

----------------------------------------------------------------
-- Stealth hook: kick block + silent + hitbox radius
----------------------------------------------------------------
...
connect(Players.PlayerRemoving, function(plr)
    ...
    if e then releaseEntry(e) if e.chams then pcall(function() e.chams:Destroy() end) end espEntries[plr] = nil end
end)
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        if Settings.ESP.Chams then
            if not e.chams or e.chams.Parent ~= char then
                ...
                e.chams.FillColor = color e.chams.Enabled = true
            end
        elseif e.chams then e.chams.Enabled = false end
    end
end
bindRender("ui_esp_" .. math.random(10000, 99999), Enum.RenderPriority.Camera.Value + 2, updateESP)
...
local function updateRadar()
    ...
    radar.bg.Size = Vector2.new(size, size) radar.bg.Position = Vector2.new(cx, cy - size * 0.5) radar.bg.Visible = true
    radar.border.Size = Vector2.new(size, size) radar.border.Position = Vector2.new(cx, cy - size * 0.5) radar.border.Visible = true
    local root = getRoot()
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        local px, py = rel.X * scale, -(-rel.Z) * scale
...
bindRender("ui_rad_" .. math.random(10000, 99999), Enum.RenderPriority.Camera.Value + 3, updateRadar)

----------------------------------------------------------------
-- Watermark + player list
----------------------------------------------------------------
local wmText = nil
if drawingAvailable then
    local ok, t = pcall(function() return Drawing.new("Text") end)
    if ok and t then wmText = t t.Size = 13 t.Outline = true t.Center = false t.Color = Color3.fromRGB(255, 122, 122) t.Visible = false table.insert(extraDrawings, t) end
end
...
local function updateWatermark(now)
    ...
    local up = math.floor(os.clock() - (startClock or 0))
...
local function updatePlayerList(now)
    ...
    for _, plr in ipairs(Players:GetPlayers()) do
        ...
        list[#list + 1] = { plr = plr, hp = hp, dist = dist, tool = tool and tool.Name or "-", enemy = isEnemy(plr) }
    end
    table.sort(list, function(a, b)
        local da = a.dist < 0 and 999999 or a.dist
        local db = b.dist < 0 and 999999 or b.dist
        return da < db
    end)
    for i = 1, 12 do
        local row = plRows[i] or makeRow()
        plRows[i] = row
        local d = list[i]
        if d then
            row.f.Visible = true
            ...
            row.info.Text = string.format("%s | %dhp | %dm", d.tool, d.hp, d.dist < 0 and 0 or d.dist)
        else
            row.f.Visible = false
        end
    end
...
----------------------------------------------------------------
-- Movement / combat / misc heartbeat
----------------------------------------------------------------
...
local function applyFullbright(on)
    ...
    elseif lightOrig then
        Lighting.Brightness = lightOrig.B Lighting.ClockTime = lightOrig.T Lighting.FogEnd = lightOrig.F Lighting.GlobalShadows = lightOrig.G
        lightOrig = nil
    end
end
...
connect(RunService.Heartbeat, function(dt)
    ...
    if Settings.Combat.KillAura and now >= auraNext then
        ...
        if best and tool and isVisible(best.Position, best.Parent) then
...
----------------------------------------------------------------
-- FullUnload (до wiring, чтобы кнопки его видели)
----------------------------------------------------------------
local unloaded = false
local function FullUnload()
    if unloaded then return end
    unloaded = true
    for _, name in ipairs(renderSteps) do pcall(function() RunService:UnbindFromRenderStep(name) end) end
...
TabMove:CreateDropdown({ Name = "Bypass Mode", Options = { "WalkSpeed", "Loop", "Velocity", "CFrame" }, CurrentOption = Settings.Speed.Mode, Callback = function(v) Settings.Speed.Mode = v end })
...
TabMove:CreateDropdown({ Name = "Fly Mode", Options = { "LinearVelocity", "CFrame" }, CurrentOption = Settings.Fly.Mode, Callback = function(v) Settings.Fly.Mode = v end })
...
TabMisc:CreateButton({ Name = "Pull Remote Config", Callback = function()
    ...
    if not body then
        ...
        if ok and type(r) == "string" then body = r end
    end
    if body then
        local ok, data = pcall(function() return HttpService:JSONDecode(body) end)
        if ok and type(data) == "table" then
            mergeConfig(Settings, data)
            cfgUrl:Set("remote config: merged")
            queueSave()
            return
        end
    end
...
local destroyBtn
destroyBtn = TabMisc:CreateButton({ Name = "ПОЛНОЕ САМОУНИЧТОЖЕНИЕ", Callback = function()
    task.spawn(function()
        for i = 5, 1, -1 do
...
local function flipKey(path, obj)
    local t = Settings[path]
    if not t then return end
    t.Enabled = not t.Enabled
    if obj then obj:Set(t.Enabled) end
    queueSave()
end
...
connect(UserInputService.InputBegan, function(input, processed)
    ...
    if k == K.Aimbot then
        flipKey("Aimbot", ui.Aimbot)
    elseif k == K.ESP then
        flipKey("ESP", ui.ESP)
    elseif k == K.Fly then
        flipKey("Fly", ui.Fly)
    elseif k == K.Speed then
        flipKey("Speed", ui.Speed)
    end
end)
...
print("[unknown] v2.1 fixed — loaded ok")
