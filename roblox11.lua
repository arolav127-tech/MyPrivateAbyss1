...
if not LP then warn("[ABYSS] No LocalPlayer; abort.") return end
...
local S={
	...
	Move={Speed=false,SpeedMode="Walk",SpeedValue=50,Fly=false,FlyValue=60,NoClip=false,InfJump=false,Bhop=false,Hitbox=false,HitboxSize=12,Spinbot=false,SpinSpeed=25},
	...
	Misc={Watermark=true,AntiAFK=true,PanicKey="End"},
	Keys={UI="RightShift",Aimbot="X",Silent="B",Fly="F",Speed="V"},
}

-- Config (type-safe load/save)
local CFGPATH="AbyssFW/unknown_v14.json"
local function loadConfig()
	...
	for g,t in pairs(S) do
		if type(data[g])=="table" then
			for k in pairs(t) do
				local v=data[g][k]
				if type(v)==type(t[k]) then t[k]=v end
			end
		end
	end
end
...
-- SAFE GUI parent: gethui(validated) -> PlayerGui(timeout) -> nil (NO CoreGui)
local function safeGuiParent()
	...
	if ok and pg and pg:IsA("PlayerGui") then return pg end
	return nil
end
local guiParent=safeGuiParent()
if not guiParent then
	warn("[ABYSS] No safe GUI parent (gethui/PlayerGui). UI disabled; modules active via keybinds.")
end
...
local function wire(plr)
	...
	rec.pscope:Connect(plr:GetPropertyChangedSignal("Team"),function() recomputeEnemy(rec) end)
	rec.pscope:Connect(plr:GetPropertyChangedSignal("Neutral"),function() recomputeEnemy(rec) end)
...
App:Connect(LP.CharacterAdded,function(char)
	local rec=Cache.byPlayer[LP]
	if rec then onCharAdded(rec,char) end
	MV.origWalk=nil
	destroyFly()
	MV.ncOn=false
...
local function acquire(k)
	...
	if not o then local ok,d=pcall(Drawing.new,k) if not ok then return nil end o=d end
...
-- Movement state + fly lifecycle (balanced)
MV={origWalk=nil,fly=nil,ncOn=false,lastJump=0,lastBhop=0}
function destroyFly()
	if MV.fly then
...
local function hookBody(self,...)
	...
	local isLegacy=(m=="FindPartOnRay" or m=="FindPartOnRayWithIgnoreList")
...
local function installHook()
	...
	if not ok or not gmt then warn("[ABYSS] Silent unsupported: no metatable access") return end
...
local function isVisible(pos,targetChar)
	...
	local rp=RaycastParams.new() rp.FilterType=Enum.RaycastFilterType.Exclude rp.IgnoreWater=true rp.FilterDescendantsInstances=ignore
	local origin=cam.CFrame.Position local dir=pos-origin
	if dir.Magnitude<0.05 then return true end
	internalRay=true
	local ok,hit=pcall(WS.Raycast,WS,origin,dir,rp)
	internalRay=false
		...
		if plr~=LP and rec.alive and rec.char then
			if not S.Aimbot.OnlyEnemies or rec.enemy then
				...
				if part then
					...
					if S.Aimbot.Prediction>0 then local vel=part.AssemblyLinearVelocity if typeof(vel)=="Vector3" then if vel.Magnitude>500 then vel=vel.Unit*500 end pos=pos+vel*S.Aimbot.Prediction end end
...
-- FOV circle
local fovCircle=nil
if CAP.drawing then local ok,c=pcall(Drawing.new,"Circle") if ok and c then fovCircle=c c.Thickness=2 c.NumSides=64 c.Filled=false c.Visible=false App:Add(function() pcall(function() c:Remove() end) end) end end
...
-- ESP render step (pooled)
local ESPS={}
local function releaseViz(rec) local v=ESPS[rec] if not v then return end if v.box then release(v.box,"Square") v.box=nil end if v.hpBg then release(v.hpBg,"Square") v.hpBg=nil end if v.hpFill then release(v.hpFill,"Square") v.hpFill=nil end if v.snap then release(v.snap,"Line") v.snap=nil end if v.name then release(v.name,"Text") v.name=nil end if v.dist then release(v.dist,"Text") v.dist=nil end if v.chams then pcall(function() v.chams:Destroy() end) v.chams=nil end end
...
App:Connect(RS.Heartbeat,function(dt)
	...
	-- Bhop
	if S.Move.Bhop and UIS:IsKeyDown(Enum.KeyCode.Space) then local st=hum:GetState() if st==Enum.HumanoidStateType.Landed or st==Enum.HumanoidStateType.Running then hum:ChangeState(Enum.HumanoidStateType.Jumping) end end
	-- Spinbot
	if S.Move.Spinbot then root.CFrame=root.CFrame*CFrame.Angles(0,math.rad(S.Move.SpinSpeed),0) end
	-- Fly
	if S.Move.Fly then
	...
	-- AA
	local aj=0
	if S.AA.Jitter then aj=math.sin(tick()*12)*math.rad(S.AA.JitterAngle) end
	local des=0
	if S.AA.Desync then if S.AA.DesyncMode=="Spin" then des=math.rad(tick()*S.AA.DesyncSpeed) else des=math.rad(60) end end
	local joint=root:FindFirstChild("RootJoint")
	if joint and joint:IsA("Motor6D") then
		if not MV.origRootC0 then MV.origRootC0=joint.C0 end
		if S.AA.Jitter or S.AA.Desync then joint.C0=MV.origRootC0*CFrame.Angles(0,aj+des,0) elseif joint.C0~=MV.origRootC0 then joint.C0=MV.origRootC0 end
	end
	-- FakeLag (velocity freeze pulse)
	if S.AA.FakeLag then local cyc=1/math.max(S.AA.FakeLagFrequency,0.1) local ph=(tick()%cyc) if ph<math.min(S.AA.FakeLagIntensity/60,cyc*0.7) then pcall(function() root.AssemblyLinearVelocity=Vector3.zero root.AssemblyAngularVelocity=Vector3.zero end) end end
	-- Triggerbot
	if S.Trigger.Enabled and cachedTarget and cachedTarget.rec.alive then
		if not S.Trigger.OnlyEnemies or cachedTarget.rec.enemy then
			if not S.Trigger.WallCheck or isVisible(cachedTarget.pos,cachedTarget.rec.char) then
				if CAP.mouse1 then pcall(mouse1click) end
			end
		end
	end
	-- AntiAFK
	if S.Misc.AntiAFK and UIS:IsKeyDown(Enum.KeyCode.Space)==false and math.random(1,600)==1 then pcall(function() VirtualUser:CaptureController() VirtualUser:ClickButton2(Vector2.new()) end) end
end)

-- UI (only if safe parent)
local UI=nil
if guiParent then
	...
	if gui.Parent==nil then warn("[ABYSS] GUI parenting failed; UI disabled.") else
		UI={gui=gui}
		-- (UI elements wiring omitted here for brevity in audit build; toggles/sliders bind to S via callbacks)
	end
end

-- Keybinds + panic (work regardless of UI)
App:Connect(UIS.InputBegan,function(input,processed)
	if processed then return end
	local kc=input.KeyCode.Name
	if kc==S.Keys.UI and UI and UI.gui then UI.gui.Enabled=not UI.gui.Enabled return end
	if kc==S.Misc.PanicKey then
		S.Aimbot.Enabled=false S.Aimbot.Silent=false S.Trigger.Enabled=false S.ESP.Enabled=false
		S.Move.Speed=false S.Move.Fly=false S.Move.NoClip=false S.Move.InfJump=false S.Move.Bhop=false S.Move.Spinbot=false
		S.AA.Jitter=false S.AA.Desync=false S.AA.HideHead=false S.AA.FakeLag=false
		destroyFly()
		return
	end
	if kc==S.Keys.Aimbot then S.Aimbot.Enabled=not S.Aimbot.Enabled end
	if kc==S.Keys.Silent then S.Aimbot.Silent=not S.Aimbot.Silent if S.Aimbot.Silent then installHook() end end
	if kc==S.Keys.Fly then S.Move.Fly=not S.Move.Fly end
	if kc==S.Keys.Speed then S.Move.Speed=not S.Move.Speed end
end)
...
local function Unload()
	...
	if lc and lc.char then pcall(function() for _,part in ipairs(lc.char:GetDescendants()) do if part:IsA("BasePart") then part.CanCollide=true end end end) end
	local lc2=localRec()
	if lc2 and lc2.root then local j=lc2.root:FindFirstChild("RootJoint") if j and MV.origRootC0 then pcall(function() j.C0=MV.origRootC0 end) end end
	if UI and UI.gui then pcall(function() UI.gui:Destroy() end) end
	ENV.__ABYSS=nil
end
ENV.__ABYSS={Unload=Unload}
print("[ABYSS] v14 audited ready")
